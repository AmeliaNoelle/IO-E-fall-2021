...
The user can also control the pitch of two oscillators by
moving their left and right wrists on the x axis
...
function draw() {
  ...
  if (poses.length > 0) {
    ...
    var freq = map(rightWrist.x, 0, width, 40, 880);    
    osc1.freq(freq);
    var freq2 = map(leftWrist.x, 0, width, 40, 880);    
    osc2.freq(freq2);
